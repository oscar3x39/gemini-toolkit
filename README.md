...
- 每筆對話右側出現勾選框,勾選要刪的
- 左下操作列:關鍵字框 + `選符合` / `全選` / `清除` / `刪除選取`(顯示已選數量)
- **關鍵字過濾**:輸入字串按 `選符合`(或 Enter),自動勾選標題含該字的對話(不分大小寫、累加)。適合清「特定主題」的一整批舊對話。
- 按「刪除選取」→ 二次確認 → 逐筆走 Gemini 原生「三點選單 → 刪除 → 確認」流程
...
- 可擴充方向:關鍵字過濾後全選、依日期範圍選取、刪除前匯出對話標題備份、快捷鍵。
